...
#define THING_SPEAK_KEY_STR     "6GR5RJBIELUMN358" /* API key */
#define THING_SPEAK_CHANNEL     51825 /* channel ID */
...
uint8_t THINK_SendValue(int32_t value, const CLS1_StdIOType *io) {
  ...
  uint8_t res = ERR_OK;
  uint8_t buf[64];
  uint8_t ch_id = 4; // was 4
  //CLS1_SendStr("let's connect\r\n", io->stdOut); ///////////////////////////////////////////////////////
  res = ESP_SetNumberOfConnections(1, io, ESP_DEFAULT_TIMEOUT_MS); // nof was 1
  if (res==ERR_OK) {
      ...
      if (res==ERR_OK) {
        ...
        if (res!=ERR_OK) {
          ...
          for(;;) { /* breaks */
            ...
            if (UTIL1_strncmp(buf, "SEND FAIL\r\n", sizeof("SEND FAIL\r\n")-1) == 0){  /* Fail from module *///////////////////////////////////
                                                      break;}
