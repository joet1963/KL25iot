...
extern word intTemp;
extern char display[9];

extern uint8_t scrollDisplay(char *stream, const CLS1_StdIOType *io);

char abcd[] = "test scrolling function. Is it smooth ";
...
void APP_Run(void) {
  ...
  for(;;) {
    ...
    WAIT1_Waitms(10);
    LEDG_Neg();
    //if(send){ (void)THINK_SendValue((int32_t)32, io); send = 0; }
   // if(send)  (void)scrollDisplay(abcd, io);
    send = 0;
  }
}
